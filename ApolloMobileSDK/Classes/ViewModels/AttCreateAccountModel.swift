...
final class AttCreateAccountModel: ObservableObject {
    ...
    @Published var user = AttUser() {
        didSet {
            
            if oldValue.address.country != user.address.country {
                languages = user.address.country.languages
    ...
    private(set) var languages: [AttCountry.Language] = []
...
// MARK: - Public
extension AttCreateAccountModel {
